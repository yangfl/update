...
int AmlUsbReadLargeMem (struct AmlUsbRomRW *rom) {
  ...
  unsigned short cksum = checksum(
    (unsigned short *) rom->buffer, rom->bufferLen);
#else
  /* read direction: the data is not in the buffer yet; the original tool
     sent the checksum of the previous chunk (or uninitialized memory),
     which the device cannot use to verify what it is about to send */
...
int AmlUsbReadMemCtr (struct AmlUsbRomRW *rom) {
  ...
  char *buf = rom->buffer;
  while (remain > 0) {
    unsigned int transferSize = aml_min(remain, 64u);
...
#ifdef BUGFIX
    if (fread(buf, 1, transferSize, stream) != transferSize) {
...
int AmlWriteMedia (struct AmlUsbRomRW *rom) {
  ...
  for (int addr = 0; addr <= 2; addr++) {
    ...
    for (time_t curTime = startTime; curTime - startTime < 12 * 60 * 1000;
      ...
      if (usbReadFile(&drv, buf, 256, &actualLen, NULL) == 0) {
        aml_printf("[AmlUsbRom]Err:");
