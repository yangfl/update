...
#ifdef BUGFIX
    if (fread(buf, 1, bulkSize, stream) != bulkSize) {
      aml_printf("Fail in read file at offset 0x%llx\n",
...
#ifdef BUGFIX
  if (nBytes < 0 || startOffset < 0 || startOffset + nBytes > 64) {
    aml_printf("[update]ERR(L%d):", 526);
    aml_printf("invalid startOffset %d or nBytes %d\n", startOffset, nBytes);
    return -527;
  }
#endif
  if (nBytes > 64) {
...
#ifdef BUGFIX
      if (actualLen == 0) {
        aml_printf("[update]ERR(L%d):", 641);
        aml_printf("device returned 0 bytes\n");
        ret = -641;
        goto end;
      }
#endif
...
#ifdef BUGFIX
      if (fread(buf + 8, 1, bulkSize, stream) != bulkSize) {
        aml_printf("Fail in read file at offset 0x%x\n", nBytes);
        ret = -734;
        goto end;
      }
#else
...
static int update_sub_cmd_tplcmd (struct AmlUsbRomRW &rom, const char *cmd) {
  ...
  memcpy(buf, cmd,
    ...
    strlen(cmd) + 1
#else
    aml_min(strlen(cmd), 66)
#endif
  );
  buf[66] = 1;

  unsigned int actualLen;
  struct AmlUsbRomRW romTpl = {rom.device, 68, buf, &actualLen, 0};
...
help:
  ...
  if (argc <= 2) {
    ...
    if (strArgDevLen >= 3 && memcmp(strArgDev, "dev", 3) == 0) {
      ...
      std::string devStr = strArgDev.substr(5, -1);
      char *devPath = (char *) devStr.c_str();
#else
...
#ifdef BUGFIX
    if (dev_no < 0 || dev_no >= 8 ||
        candidateDevices[dev_no] == NULL ||
        candidateDevices[dev_no][0] == '\0') {
      puts("没有此设备[设备号]");
      aml_scan_close();
      goto end;
    }
  ...
  if (strcmp(cmd, "bulkcmd") == 0) {
    char buf[128] = {};
    memcpy(buf, argv[argc - 1],
#ifndef BUGFIX
      strlen(argv[argc - 1])
#else
      aml_min(strlen(argv[argc - 1]), 66)
#endif
    );
    buf[66] = 1;
  ...
  if (strcmp(cmd, "down") == 0) {
    ...
    if (update_scan((void **) &rom.device, 0,
#ifdef BUGFIX
          dev_no,
#else
          0,
...
#ifdef BUGFIX
      const char *downFile = argv[argc == 4 ? 3 : 2];
#else
      const char *downFile = argv[3];
#endif
      if (WriteMediaFile(&rom, downFile) != 0) {
        aml_printf("ERR:write data to media failed\n");
      }
    } else {
