...
#ifdef BUGFIX
    if (ret < 0) {
      aml_printf("Ctrl_RdWr failed at 0x%x, ret=%d\n", addr, ret);
      break;
    }
#endif
    processed += chunk;
...
static int _Aml_Libusb_bl2_csw (
  ...
  int ret;
  if (doSend) {
    ret = usb_bulk_write(handle, AM_EP_OUT, csw, sizeof(csw), 30000);
  } else {
    ret = usb_bulk_read(handle, AM_EP_IN, csw, sizeof(csw), 30000);
  }
...
static int _Aml_Libusb_bl2_cmd_parser (
  ...
  if (cbw[17] == 0) {
#ifdef BUGFIX
    *cmd = (signed char) cbw[16] >= 0 ? 0xABCD : 0xABCE;
#else
  ...
  if (*cmd == 0xABCD) {
    ...
    ret = _Aml_Libusb_bl2_csw(dev, true, *cmd == 0xABCF, NULL);
  }
